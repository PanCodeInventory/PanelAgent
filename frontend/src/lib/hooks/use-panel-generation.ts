...
export interface UsePanelGenerationReturn {
  ...
  generate: (markers: string[], inventoryFile: string) => Promise<void>;
  clear: () => void;
}
...
export function usePanelGeneration(): UsePanelGenerationReturn {
  ...
  const generate = useCallback(
    async (markers: string[], inventoryFile: string) => {
      ...
      try {
        ...
        if (
          (data.candidates?.length === 0 || data.status === "error") &&
          trimmedMarkers.length > 0
        ) {
